...
resource "aws_autoscaling_group" "workers" {
  count                   = "${var.module_enabled && var.worker_group_count ? var.worker_group_count :0}"
  ...
  tags = ["${concat(
    list(
      ...
      map("key", "k8s.io/cluster-autoscaler/${lookup(var.worker_groups[count.index], "autoscaling_enabled", local.workers_group_defaults["autoscaling_enabled"]) == 1 ? "enabled" : "disabled"  }", "value", "true", "propagate_at_launch", true),
...
resource "aws_security_group" "workers" {
  count       = "${var.module_enabled && var.worker_create_security_group ? 1 : 0}"
...
resource "aws_security_group_rule" "workers_ingress_cluster_kubelet" {
  count                    = "${var.module_enabled && var.worker_create_security_group ? (var.worker_sg_ingress_from_port > 10250 ? 1 : 0) : 0}"
...
resource "aws_iam_role" "workers" {
  count                 = "${var.module_enabled && var.manage_worker_iam_resources ? 1 : 0}"
  name_prefix           = "${aws_eks_cluster.this.name}"
...
resource "aws_iam_instance_profile" "workers" {
  count       = "${var.module_enabled && var.manage_worker_iam_resources ? var.worker_group_count : 0}"
  name_prefix = "${aws_eks_cluster.this.name}"
...
resource "aws_iam_role_policy_attachment" "workers_additional_policies" {
  count      = "${var.module_enabled && var.manage_worker_iam_resources ? var.workers_additional_policies_count : 0}"
  role       = "${aws_iam_role.workers.name}"
