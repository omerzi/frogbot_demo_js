resource "local_file" "config_map_aws_auth" {
  count    = "${var.module_enabled && var.write_aws_auth_config ? 1 : 0}"
  content  = "${data.template_file.config_map_aws_auth.rendered}"
  filename = "${var.config_output_path}config-map-aws-auth_${var.cluster_name}.yaml"
}

resource "null_resource" "update_config_map_aws_auth" {
  count      = "${var.module_enabled && var.manage_aws_auth ? 1 : 0}"
...
data "template_file" "launch_template_mixed_worker_role_arns" {
  count    = "${var.module_enabled && var.worker_group_launch_template_mixed_count ? var.worker_group_launch_template_mixed_count : 0}"
...
data "template_file" "launch_template_worker_role_arns" {
  count    = "${var.module_enabled && var.worker_group_launch_template_count ? var.worker_group_launch_template_count :0}"
  template = "${file("${path.module}/templates/worker-role.tpl")}"
...
data "template_file" "map_users" {
  count    = "${var.module_enabled && var.map_users_count ? var.map_users_count :0 }"
...
data "template_file" "map_roles" {
  count    = "${var.module_enabled && var.map_roles_count ? var.map_roles_count :0}"
...
data "template_file" "map_accounts" {
  count    = "${var.module_enabled && var.map_accounts_count ? var.map_accounts_count :0 }"
