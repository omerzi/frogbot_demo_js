...
sed -i '/^KUBELET_EXTRA_ARGS=/a KUBELET_EXTRA_ARGS+=" --eviction-hard memory.available<1Gi --eviction-soft memory.available<2Gi --eviction-soft-grace-period memory.available=2m0s --kube-reserved cpu=500m,memory=9000Mi --system-reserved cpu=500m,memory=1000Mi"' /etc/eks/bootstrap.sh
