...
resource "aws_route_table" "private_subnet_secondary_cidr" {
  count = var.create_vpc && var.create_secondary_cidr_route_table ? local.nat_gateway_count : 0

  vpc_id = local.vpc_id

  tags = merge(
    {
      "Name" = var.single_nat_gateway ? "${var.name}-${var.private_subnet_secondary_cidr_suffix}" : format(
        "%s-${var.private_subnet_secondary_cidr_suffix}-%s",
        var.name,
        element(var.azs, count.index),
      )
    },
...
#################
# Database secondary routes
#################
resource "aws_route_table" "database_secondary" {
  count = var.create_vpc && var.create_secondary_database_subnet_route_table && length(var.database_secondary_subnets) > 0 ? 1 : 0
...
resource "aws_subnet" "private" {
  count = var.create_vpc || var.is_sub_region  && length(var.private_subnets) > 0 ? length(var.private_subnets) : 0
  vpc_id                          =  var.is_sub_region ? var.vpc_self_link : local.vpc_id
...
resource "aws_subnet" "private_subnet_secondary_cidr" {
  count = var.create_vpc || var.is_sub_region  && length(var.private_subnet_secondary_cidr) > 0 ? length(var.private_subnet_secondary_cidr) : 0
  vpc_id                          =  var.is_sub_region ? var.vpc_self_link : local.vpc_id
  cidr_block                      = var.private_subnet_secondary_cidr[count.index]
  availability_zone               = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) > 0 ? element(var.azs, count.index) : null
  availability_zone_id            = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) == 0 ? element(var.azs, count.index) : null
  assign_ipv6_address_on_creation = var.private_subnet_assign_ipv6_address_on_creation == null ? var.assign_ipv6_address_on_creation : var.private_subnet_assign_ipv6_address_on_creation
  ...
  tags = merge(
    ...
    var.tags,
    var.private_secondary_cidr_subnet_tags,
  )
}
##################
# Database subnet
...
resource "aws_subnet" "database_secondary" {
  count = var.create_vpc && length(var.database_secondary_subnets) > 0 ? length(var.database_secondary_subnets) : 0

  vpc_id                          = local.vpc_id
  cidr_block                      = var.database_secondary_subnets[count.index]
  availability_zone               = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) > 0 ? element(var.azs, count.index) : null
  availability_zone_id            = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) == 0 ? element(var.azs, count.index) : null
  assign_ipv6_address_on_creation = var.database_secondary_subnet_assign_ipv6_address_on_creation == null ? var.assign_ipv6_address_on_creation : var.database_secondary_subnet_assign_ipv6_address_on_creation

  ipv6_cidr_block = var.enable_ipv6 && length(var.database_secondary_subnet_ipv6_prefixes) > 0 ? cidrsubnet(aws_vpc.this[0].ipv6_cidr_block, 8, var.database_secondary_subnet_ipv6_prefixes[count.index]) : null

  tags = merge(
    {
      "Name" = format(
        "%s-${var.database_secondary_subnet_suffix}-%s",
        var.name,
        element(var.azs, count.index),
      )
    },
...
resource "aws_db_subnet_group" "database" {
  ...
  subnet_ids  = length(var.db_subnetgroup_override)> 0 ? var.db_subnetgroup_override : aws_subnet.database.*.id
...
resource "aws_db_subnet_group" "database_secondary" {
  count = var.create_vpc && length(var.database_secondary_subnets) > 0 ? 1 : 0

  name        = lower("${var.name}-secondary")
  description = "Database secondary subnet group for ${var.name}"
  subnet_ids  = length(var.db_subnetgroup_override)> 0 ? var.db_subnetgroup_override : aws_subnet.database_secondary.*.id

  tags = merge(
    {
      "Name" = format("%s", "${var.name}-secondary")
    },
    var.tags,
    var.database_subnet_group_tags,
  )
...
resource "aws_default_network_acl" "this" {
  ...
  subnet_ids = setsubtract(
    compact(flatten([
      ...
      aws_subnet.intra.*.id,
      aws_subnet.private_subnet_secondary_cidr.*.id,
...
resource "aws_route" "private_subnet_secondary_cidr_nat_gateway" {
  count = var.create_vpc && var.create_secondary_cidr_route_table && var.enable_nat_gateway ? local.nat_gateway_count : 0

  route_table_id         = element(aws_route_table.private_subnet_secondary_cidr.*.id, count.index)
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id         = element(aws_nat_gateway.this.*.id, count.index)
...
resource "aws_route" "private_subnet_secondary_cidr_ipv6_egress" {
  count = var.create_vpc && var.create_secondary_cidr_route_table && var.enable_ipv6 ? length(var.private_subnets) : 0
...
resource "aws_route_table_association" "private" {
  ...
  route_table_id = var.is_sub_region  ? var.private_route_table_ids[count.index] : element(
    aws_route_table.private.*.id,
    var.single_nat_gateway ? 0 : count.index,
  )
}

resource "aws_route_table_association" "private_subnet_secondary_cidr" {
  count = var.create_vpc || var.is_sub_region && length(var.private_subnet_secondary_cidr) > 0 ? length(var.private_subnet_secondary_cidr) : 0

  subnet_id = element(aws_subnet.private_subnet_secondary_cidr.*.id, count.index)
  route_table_id = var.is_sub_region && var.create_secondary_cidr_route_table ==false ? var.private_secondary_route_table_ids[count.index] : element(
    aws_route_table.private_subnet_secondary_cidr.*.id,
    var.single_nat_gateway ? 0 : count.index,
  )
}
...
resource "aws_route_table_association" "database_secondary" {
  ...
  subnet_id = element(aws_subnet.database_secondary.*.id, count.index)
  route_table_id = element(
    coalescelist(aws_route_table.database_secondary.*.id, aws_route_table.private.*.id),
    var.single_nat_gateway || var.create_secondary_database_subnet_route_table ? 0 : count.index,
  )
}
...
# resource "aws_route" "cloud_wan_route" {
#   count = var.create_vpc && var.connect_to_global_network ? local.nat_gateway_count : 0

#   route_table_id         = length(var.private_subnet_secondary_cidr) > 0 ? element(aws_route_table.private_subnet_secondary_cidr.*.id, count.index) : element(aws_route_table.private.*.id, count.index)
#   destination_cidr_block = var.extended_vpcs_cidr
#   core_network_arn = var.core_network_arn
#   timeouts {
#     create = "5m"
#   }
#   depends_on = [
#     aws_networkmanager_attachment_accepter.cloud_wan_vpc_attachment_accepter[0]
#   ]
# }

resource "aws_route" "cloud_wan_route_prefix" {
  count = var.create_vpc && var.connect_to_global_network ? local.nat_gateway_count : 0

  route_table_id         = length(var.private_subnet_secondary_cidr) > 0 ? element(aws_route_table.private_subnet_secondary_cidr.*.id, count.index) : element(aws_route_table.private.*.id, count.index)
  destination_prefix_list_id = aws_ec2_managed_prefix_list.cloud_wan[0].id
  core_network_arn = var.core_network_arn
  timeouts {
    create = "5m"
  }
...
resource "aws_ec2_managed_prefix_list" "cloud_wan" {
  count = var.connect_to_global_network ? 1 : 0 
  name           = "All VPC CIDR-s"
  address_family = "IPv4"
  max_entries    = 5

  tags = {
    Name = "cloudWan Prefix"
  }
}

resource "aws_ec2_managed_prefix_list_entry" "extended_cidrs" {
  count          = var.connect_to_global_network ? length(var.aws_cloud_wan_prefix_cidrs) : 0
  cidr           = element(concat(var.aws_cloud_wan_prefix_cidrs, [""]), count.index)
  description    = "CloudWan cidr ranges"
  prefix_list_id = aws_ec2_managed_prefix_list.cloud_wan[0].id
...
resource "aws_networkmanager_vpc_attachment" "cloud_wan_vpc_attachment" {
  count = var.connect_to_global_network ? 1 : 0 
  core_network_id = var.core_network_id
  subnet_arns     = aws_subnet.private_subnet_secondary_cidr.*.arn
  vpc_arn         = aws_vpc.this[0].arn
  tags ={
  Name  = var.name,
  env = var.environment
  } 
}
resource "aws_networkmanager_attachment_accepter" "cloud_wan_vpc_attachment_accepter" {
  count = var.connect_to_global_network ? 1 : 0 
  attachment_id   = aws_networkmanager_vpc_attachment.cloud_wan_vpc_attachment[0].id
  attachment_type = "VPC"
    depends_on = [
    aws_networkmanager_vpc_attachment.cloud_wan_vpc_attachment[0]
  ]
}